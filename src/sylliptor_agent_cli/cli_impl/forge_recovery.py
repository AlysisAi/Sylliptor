...
def _lock_targets(path: Path) -> tuple[Path, list[tuple[str, Path]], Any]:
    ...
    try:
        ...
        paths = None
    runtime_dir = paths.runtime_dir if paths is not None else root / ".sylliptor"
...
def forge_unlock(
    ...
    if status_after is not None:
        ...
        if status_after in RESUMABLE_RUN_STATUSES:
            console.print("Continue this run with: sylliptor forge resume --path .")
...
def _resume_blocker(status: str) -> str | None:
    ...
    if status == RUN_STATUS_RUNNING:
        return (
            ...
            "Wait for it to finish, or inspect the lock with `sylliptor forge unlock`."
        )
    if status == RUN_STATUS_COMPLETED:
        return "this run already completed; start a new one with `sylliptor forge plan`."
    if status == RUN_STATUS_APPROVED:
        return (
            "this run was never interrupted. Execute it with `sylliptor forge run` "
            "instead of resuming."
        )
