...
def test_chat_stream_command_toggles_for_following_turn(tmp_path: Path, monkeypatch) -> None:
    runner = CliRunner()
    captured: dict[str, bool] = {}
...
def test_chat_history_no_session_artifacts(tmp_path: Path, monkeypatch) -> None:
    runner = CliRunner()
    ...
    assert result.exit_code == 0
    assert "No session artifacts found yet." in result.output


def test_chat_history_blocked_in_forge_mode(tmp_path: Path, monkeypatch) -> None:
    runner = CliRunner()
    ...
    assert result.exit_code == 0
    assert "History search is disabled in Forge." in result.output


def test_chat_history_no_matches(tmp_path: Path, monkeypatch) -> None:
    runner = CliRunner()

    session_dir = tmp_path / ".sylliptor" / "sessions" / "sid" / "history"
    session_dir.mkdir(parents=True, exist_ok=True)
    (session_dir / "chunk_0001.jsonl").write_text(
        '{"idx":1,"message":{"content":"hello world"}}\n',
        encoding="utf-8",
    )
    ...
    assert result.exit_code == 0
    assert "No history matches found." in result.output


def test_chat_history_truncated_results(tmp_path: Path, monkeypatch) -> None:
    runner = CliRunner()
    history_dir = tmp_path / ".sylliptor" / "sessions" / "sid" / "history"
    history_dir.mkdir(parents=True, exist_ok=True)
    (history_dir / "chunk_0001.jsonl").write_text(
        "\n".join(
            f'{{"idx":{i},"message":{{"content":"MUST keep requirement {i}"}}}}' for i in range(80)
        )
        + "\n",
        encoding="utf-8",
    )
    ...
    assert result.exit_code == 0
    assert "History Matches (50)" in result.output
    assert "Results truncated at max_results." in result.output
