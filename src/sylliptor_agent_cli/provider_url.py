...
_SYLLIPTOR_TRIAL_PROXY_PATH = "/functions/v1/llm"
...
def known_provider_key_from_base_url(base_url: str | None) -> str | None:
    ...
    # The Sylliptor hosted proxy (the `llm` Supabase Edge Function) forwards to
    ...
    if _SYLLIPTOR_TRIAL_PROXY_PATH in path and (
        host == "supabase.co" or host.endswith(".supabase.co")
    ):
        return "deepseek"
    # Reserved hostname for a future dedicated gateway box — also DeepSeek.
    if host == "api.sylliptor.alysisai.com":
        return "deepseek"
