...
def _blast_radius_gate_enabled(cfg: Any | None) -> bool:
    ...
    ``SYLLIPTOR_BLAST_RADIUS`` (off/0/false/no/disabled) wins over the config
    ...
    """
    env_value = env_get("SYLLIPTOR_BLAST_RADIUS")
...
_STATUS_SUMMARY_LINES = {
    ...
    BlastRadiusStatus.UNATTRIBUTED: (
        ...
        "clean-tree run covering them I cannot tell whether these failures are mine: {ids}. "
        "Their cause is UNATTRIBUTED — neither confirmed pre-existing nor confirmed a "
        "regression."
    ),
...
def build_blast_radius_status_summary(assessment: BlastRadiusAssessment) -> str:
    ...
    line = template.format(
        ...
        ids=_format_ids(assessment.new_failures or assessment.unattributed),
        baseline=assessment.baseline_command or "the clean-tree baseline",
    )
