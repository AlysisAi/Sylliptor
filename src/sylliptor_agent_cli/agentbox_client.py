...
class AgentBoxClient:
    ...
    def __init__(
        ...
        self.config = AgentBoxClientConfig(
            ...
            agent_id=(
                agent_id
                or os.environ.get("AGENTBOX_AGENT_ID")
                or f"{resolved_machine_id}_sylliptor"
            ),
            runtime_version=(str(runtime_version).strip() or "sylliptor")[:80],
            queue_dir=Path(
        ...
        if start_background:
            self._shipper = threading.Thread(
                ...
                name="sylliptor-agentbox-shipper",
                daemon=True,
            )
            self._shipper.start()
    ...
    def _start_heartbeat(self) -> None:
        ...
        self._heartbeat = threading.Thread(
            ...
            name="sylliptor-agentbox-heartbeat",
            daemon=True,
        )
...
def _log_client_exception(where: str, exc: BaseException) -> None:
    try:
        ...
        path = home / "sylliptor-agentbox.log"
        _rotate_log(path)
