...
def _sequential_conflict_report_lines(
    *,
    ...
    lines.append(
        f"Then continue the remaining tasks with `sylliptor forge run --path {workspace}`."
    )
    lines.append(
        "To let an agent attempt the resolution instead, re-run this task with "
        f"`sylliptor forge exec {task_id} --pr --auto-resolve-conflicts "
        f"--path {workspace}`."
    )
    return lines
...
def _stage_and_commit_task_changes(root: Path, *, message: str) -> str | None:
    ...
    stage_all(root)
    unstage_staged_prefixes(root, [".sylliptor", ".sylliptor_images", "sylliptor-feedback"])
    ensure_not_staged_prefixes(root, [".sylliptor", ".sylliptor_images", "sylliptor-feedback"])
...
_SEQUENTIAL_DELEGATE_ENV = "SYLLIPTOR_SWARM_SEQUENTIAL_DELEGATE"
...
def _swarm_sequential_delegation_blocker(
    *,
    ...
    if not isinstance(parallel, int) or parallel != 1:
        return "parallel > 1"
    if str(os.environ.get(_SEQUENTIAL_DELEGATE_ENV, "")).strip().lower() in {
        "0",
        "false",
        "no",
        "off",
...
def execute_forge_task(
    *,
    ...
    # Assigned inside the execution try below; pre-bound so the verification
    # repair loop can reuse it even if setup failed before it was built.
