...
cover the harder case where the bridge is killed before teardown runs. A later Sylliptor process
...
_DOCKER_CONTAINER_PATTERN = re.compile(r"^sylliptor-bgsbx-[0-9a-f]{12}$")
...
class TerminalOwnershipLedger:
    def __init__(self, state_dir: Path | None = None) -> None:
        configured = str(env_get("SYLLIPTOR_TERMINAL_OWNERSHIP_DIR") or "").strip()
        configured_data = str(env_get("SYLLIPTOR_DATA_DIR") or "").strip()
        data_root = (
